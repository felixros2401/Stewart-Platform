...
void calcServo(){
 // clean up X & Y
  ...
  String valX = nf(xy[0], 3, 0); //3 bits at the time
  String valY = nf(xy[1], 3, 0);  //3 bits at the time
  ...
  String valRoll = nf(rollpitch[0], 3, 0); //3 bits at the time
  String valPitch = nf(rollpitch[1], 3, 0);  //3 bits at the time
  float Roll = Float.parseFloat(valRoll);
  float Pitch = Float.parseFloat(valPitch);
